...
/**
 * 基于SVN方式的打包工具类 适用于B2B2C新产品 增量更新包
 * 
 * @author fengdm
 *
 */
public class FreePatchUtil {
	...
	public static String projectNoDirPath = "/tcl_bigmember/trunk/BigCustomer/";// 从SVN更新下来在src目录之前的项目路径

	public static String patchFile = "D:/longwork/addValueFile/patch.txt";// 将SVN更新下来的文件记录保存为patch.txt

	public static String projectPath = "D:/longwork/tcl_bigmember/trunk/BigCustomer";

	public static String webContent = "src/main/webapp";// web应用文件夹名

	public static String classPath = "D:/longwork/tcl_bigmember/trunk/BigCustomer/target/classes";// class存放路径
		...
		try {
			...
			while ((line = dr.readLine()) != null) {
				...
				if (line.indexOf(projectNoDirPath) != -1) {
					line = line.replaceAll(" ", "");
					line = line.substring(projectNoDirPath.length(),
							line.length());
					fileList.add(line);
				}
			}
		...
		for (String fullFileName : list) {
			...
			if (fullFileName.indexOf("src/main/java") != -1) {// 对源文件目录下的文件处理
				...
				String desFilePathStr = desPath + "/" + version
						+ "/ROOT/WEB-INF/classes" + tempDesPath;
				String desFileNameStr = desPath + "/" + version
						+ "/ROOT/WEB-INF/classes" + fileName;
				...
				Map<String, String> checkMaps = checkInnerJava(fullFileName,
						desFileNameStr);
				...
				String fullDesFileNameStr = desPath + "/" + version + "/ROOT/"
						+ desFileName;
				if (fullFileName.indexOf("src/main/resources") != -1) {
					fullDesFileNameStr = desPath + "/" + version
							+ "/ROOT/WEB-INF/classes" + desFileName;
				}
	 ...
	 * @return
	 */
	public static Map<String, String> checkInnerJava(String fileName,
			String desFileNameStr) {
		fileName = fileName.substring(0, fileName.indexOf(".class"));
		desFileNameStr = desFileNameStr.substring(0,
				desFileNameStr.indexOf(".class"));
		int i = 1;
		File file = null;
